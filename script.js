document.addEventListener("DOMContentLoaded", () => {
    // 음성 인식 기능 (Web Speech API) - 초기 버튼 의존 핸들러는 비활성화(아래 통합 루프 사용)
    let recognition;
    ...
    callQuestionGPT.addEventListener("click", async () => {
        ...
        try {
            ...
            if (data.choices && data.choices.length > 0) {
                ...
                // 임베딩 준비(분류 정확도 향상)
                try { await ensureQuestionEmbeddings(apiKey, questions); } catch(e) { console.warn('ensureQuestionEmbeddings failed', e); }
            } else {
...
function showInterviewEndModal() {
    ...
    const msToMinSec = (ms) => { const s=Math.max(0,Math.floor(ms/1000)), m=Math.floor(s/60); return `${m}m ${String(s%60).padStart(2,'0')}s`; };
