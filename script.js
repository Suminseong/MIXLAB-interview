    ...
    async function speakTextPersona(text, persona) {
        ...
        try {
            ...
            function cleanup(silent = false) {
                ...
                setMicStatus('듣는 중');
                try {
                    if (window.AnalyticsKit && window.AnalyticsKit.Timeline && typeof window.AnalyticsKit.Timeline.personaSpeakEnd === 'function') window.AnalyticsKit.Timeline.personaSpeakEnd();
                } catch (_) { }
                // 말하기 종료 시 정적 이미지로 복귀
                try {
                ...
                if (it.key === 'score') {
                    return `
                        <li class="emotion-card">
                            ...
                            <div class="emotion-note"><strong style="font-size:1.25rem;
color:#5872FF">${it.extra}</strong></div>
