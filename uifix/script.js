...
${conv.map(r => `Q: ${r.q}\nU: ${r.u}\nA: ${r.a}`).join('\n\n')}`);
    ...
    async function classifyQuestionIndex(userText, lastIdx, qs, key, turnCount = 0) {
        ...
        // local greeting regex (avoid depending on GREET_REGEX which may be declared later)
        const GREET_LOCAL = /(안녕|안녕하세요|처음|반갑|만나서)/i;

        // If this is an early turn or greeting, treat as icebreaking (index 0)
        if ((typeof turnCount === 'number' && turnCount <= 2) || GREET_LOCAL.test(userText)) {
            return { index: 0, score: 1, reason: 'greet/intro' };
        }

        // If user explicitly referenced a question number (eg. "3번", "질문 2"), honor it
        ...
        // Exact-match fast path: if user text exactly matches one of the questions, choose that index
        ...
        const best = sims[0], second = sims[1] || { s: 0 };
        let idx = best.i + 1;
    ...
    async function sendMessage(voiceInput, isVoice) {
        ...
        // 직전 인덱스 보관
        try {
            predicted = await classifyQuestionIndex(userText, lastIndex || 0, questions || [], key, turnCount);
        } catch (e) { }
